...
![Figure 3.1](/img/chapter3/figure3.1.mov)
...
Figure 3.4 illustrates a distributed call graph. Boxes denote executions. Nested boxes denote computations, a group of executions that share a locality. Starting from the root, executions connected by run execute in the same computation while executions connected by rpc execute in a different computation.

<br />

![Figure 3.4](/img/chapter3/figure3.4.mov)
...
![Figure 3.5](/img/chapter3/figure3.5.mov)
...
![Figure 3.6](/img/chapter3/figure3.6.mov)
...
### 3.1.3 Relevance for Agentic Applications
...
Figure 3.6 illustrates the architecture of Distributed Async Await. A deployment consists of a Resonate Server, the orchestrator, and one or more Resonate Workers, the executors:
...
![Figure 3.6](/img/chapter3/figure3.6.svg)

<div style={{textAlign: 'center', fontStyle: 'italic', marginTop: '-10px'}}>
  Figure 3.6: The Architecture of Distributed Async Await, illustrating a Resonate Server, storing Durable Promises and two Resonate Workers, running ephemeral computations.
</div>
...
The Research Agent is a graph (see Figure 3.7): The nodes in the graph represent function executions, the edges describe invoke and await relationships between function executions. An invocation is either a local invocation, a `run` or a remote invocation, an `rpc`. If the invocation is a remote invocation, the invocation partitions the graph into another computation, spawning its own recovery boundary.

<br />

![Figure 3.7](/img/chapter3/figure3.7.mov)

<div style={{textAlign: 'center', fontStyle: 'italic', marginTop: '-10px'}}>
  Figure 3.7: The call graph of the Research Agent
</div>
...
- When an agent transitions from the terminal to the cloud, the agent transitions from a non-distributed system into a distributed systems.
...
- remote invocations introduces a spatial cut (different location); await introduces a temporal cut (suspension and continuation).
